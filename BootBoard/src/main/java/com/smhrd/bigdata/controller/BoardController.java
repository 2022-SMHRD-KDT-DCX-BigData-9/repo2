...
public class BoardController {
	...
	public String write(BootBoard b, @RequestPart("photo") MultipartFile photo) {
		...
		b.setImg(newFileName); // 새로 만들어준 이름으로 img 필드 초기화
		
		service.write(b);
		return "redirect:/";
	}
	
	@GetMapping("/board/content/{idx}") // 매번 바뀐다
	public String content(@PathVariable("idx") long idx, Model model) throws IOException {
		...
		//4. -> title~img(파일이름) console창에 출력해보기
		...
		//List<E> list = new ArrayList(); // 중간 데이터 삭제 할 것 같으면 업캐스팅하고 linkedlist로 바꿔서 쓰면 된다
